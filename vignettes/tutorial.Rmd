...
We could call `print(dho$stanmodel)` to see the entire generated Stan model code.
...
```{r dho_fit_view_ode1}
ys <- dho_fit_prior$extract_odesol_df()
```

We can plot ODE solutions like so
```{r dho_fit_view_ode2, fig.width=6.5, fig.height=3.2}
dho_fit_prior$plot_odesol(alpha = 0.3)
```

We can plot the distribution of ODE solutions like so
```{r dho_fit_view_ode3, fig.width=6.5, fig.height=3.2}
dho_fit_prior$plot_odesol_dist(include_y0 = TRUE)
```

We can plot ODE solution using one draw like so
```{r dho_fit_view_ode4, fig.width=6.5, fig.height=3.2}
dho_fit_prior$plot_odesol(draw_inds = 45)
```
...
```{r dho_gq_bdf, fig.width=6.5, fig.height=3.2}
gq_bdf <- dho_fit_prior$gqs(solver = bdf(tol=1e-4), t = seq(0.5, 10, by = 0.5))
```

```{r dho_gq_mp, fig.width=6.5, fig.height=3.2}
gq_mp <- dho_fit_prior$gqs(solver = midpoint(num_steps=4), t = seq(0.5, 10, by = 0.5))
```

We can again plot ODE solution using one draw like so
```{r dho_view_gq, fig.width=6.5, fig.height=3.2}
...
```{r dho_create}
dho_post <- ode_model(N,
                 odefun_vars = list(k), 
                 odefun_body = dho_fun_body,
                 odefun_init = y0,
                 loglik_vars = list(sigma, y1_obs),
                 loglik_body = dho_loglik_body)
print(dho_post)
```
...
```{r dho_post_plot, fig.width=6.5, fig.height=3.2}
plt <- dho_fit_post$plot_odesol_dist()
...
rel <- dho_fit_post$reliability(solvers = solvers)
print(rel$metrics)
unlink("results")
```
